...
.th-cls{{left:0;z-index:110;min-width:52px;position:sticky}}
.th-day{{position:sticky;left:52px;z-index:110;min-width:38px;background:var(--navy)}}
...
.col-cls{{
  ...
  background:var(--navy);color:#fff;
  padding:0 5px;font-weight:800;font-size:.8rem;
...
.col-day{{
  position:sticky;left:52px;z-index:50;
  background:#1a3a5c;color:rgba(255,255,255,.9);
  padding:0 5px;font-weight:700;font-size:.74rem;
...
function initSub() {{
  ...
  Object.keys(SCHED).sort().forEach(t=>{{
    ...
    cb.type='checkbox'; cb.id='schk-'+t;
    cb.onchange=()=>toggleAbsent(t);
    const nameSpan=document.createElement('span');
    nameSpan.className='t-name'; nameSpan.textContent=t;
    const pillSpan=document.createElement('span');
    pillSpan.className='lpill '+pc; pillSpan.textContent=wl+'/wk';
    el.appendChild(cb); el.appendChild(nameSpan); el.appendChild(pillSpan);
    tl.appendChild(el);
  }});
}}
